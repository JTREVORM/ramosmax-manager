-- ===========================================================================
-- RamosMAX Web — Final phase — 0044: execute privileges, extended again
-- ===========================================================================
-- Same model as 0014, 0020, 0031 and 0038: deny by default, allow an explicit
-- list. PostgreSQL grants EXECUTE to PUBLIC on every new function, so the
-- revoke is repeated here and the list below is the WHOLE browser-callable
...
-- ---------------------------------------------------------------------------
-- Workstream 2 — after-hours work and cash handovers
-- ---------------------------------------------------------------------------

  -- reads and policy
  app.after_hours_policy(),
  app.after_hours_methods(),
  app.after_hours_grantable(),
  ...
  app.close_after_hours_session(uuid, text),
  app.cancel_after_hours_session(uuid, text),
  -- handovers and discrepancies
  app.submit_cash_handover(uuid, bigint, text, text),
  app.receive_cash_handover(uuid, bigint, text, text, text),
  app.review_cash_discrepancy(uuid, text),
  app.resolve_cash_discrepancy(uuid, text, text, text, boolean, boolean),
  -- housekeeping: labels and reminders only, never enforcement
  app.sweep_after_hours()
to authenticated;
...
--   app.permanent_permissions, app.require_grant_list,
--   app.require_temporary_window, app.authorization_is_live,
--   app.expected_from_payments, app.tag_after_hours,
--   app.count_on_session, app.record_payment_custody,
--   app.record_reversal_custody                         after-hours machinery
