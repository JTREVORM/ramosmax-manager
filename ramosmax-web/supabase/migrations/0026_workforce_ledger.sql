-- ===========================================================================
-- RamosMAX Web — Phase F — 0026: staff pay in the existing ledger
-- ===========================================================================
-- Allowance and payroll payments are ordinary outflows in the SAME ledger as
-- everything else. Two things change:
--
--   * the entry types are accepted, and get their own daily totals
--     (`allowancesPaidUgx`, `payrollPaidUgx` — staff pay, never revenue and
--     never an operating expense);
--   * the generic finance reversal REFUSES them. Reversing staff pay has to
--     undo what the payment applied — allowances back to approved, deduction
--     applications reversed, loss recoveries restored — so it goes through the
--     workforce functions, which do all of that in one transaction.
-- ===========================================================================
...
alter table public.finance_daily_summaries
  add column if not exists allowances_paid_ugx bigint not null default 0,
  add column if not exists payroll_paid_ugx    bigint not null default 0;
...
begin
  ...
  update public.finance_daily_summaries s
     set payments_in_ugx      = s.payments_in_ugx      + case when p_kind = 'customer_payment' then p_amount else 0 end,
         ...
         -- Staff pay: its own totals, so it is never counted as an operating
         -- expense and never as revenue.
...
/*
 * The generic finance reversal, now refusing staff pay.
 *
 * Reversing an allowance or payroll payment must also put the allowances,
 * deduction applications and loss recoveries back, so it belongs to the
 * workforce functions that know how.
 */
